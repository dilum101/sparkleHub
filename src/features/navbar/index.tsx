      ...
      <div className="bg-[#1b3a6b] text-white text-xs py-2 hidden md:block">
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          <span className="text-white/70">
            Melbourne's Professional Cleaning Specialists
          </span>
          <div className="flex items-center gap-6">
            <a
              href="tel:+61420214143"
              className="flex items-center gap-1.5 text-white/80 hover:text-[#00b5be] transition-colors font-medium"
            >
            ...
            <div className="hidden md:flex items-center gap-1">
              {navLinks.map(({ id, label }) => (
                ...
                >
                  ...
                  {active === id && (
                    <span
                      className={`block h-0.5 rounded-full mt-0.5 mx-auto w-4 transition-all duration-300 ${scrolled ? 'bg-[#00b5be]' : 'bg-[#00b5be]'}`}
                    />
                  )}
                </button>
              ))}
          ...
          >
            <div className="px-4 py-4 space-y-1">
              {navLinks.map(({ id, label }) => (
                ...
                >
                  <span
                    className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${active === id ? 'bg-white' : scrolled ? 'bg-white/30' : 'bg-gray-300'}`}
                  />
                  {label}
                </button>
              ))}
              <div
                className="pt-3 border-t mt-2 space-y-2"
                style={{
                  borderColor: scrolled ? 'rgba(255,255,255,0.15)' : '#f3f4f6',
                }}
              >
                <a
                  href="tel:+61420214143"
